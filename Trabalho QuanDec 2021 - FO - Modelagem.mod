...
#     Uma empresa multinacional é produtora de diversos artigos esportivos, que são classificados em três categorias: vestuário, calçados e acessórias. A empresa possui unidadesem diversos países, e atende a demanda em praticamente todas as regiões globais.A cada trimestre, ela refaz o seu plano de produção, podendo manter algumas unidades fechadas de acordo com o contexto atual de demandas, preços e custos, visando sempre maximizar o lucro da empresa como um todo.Cada unidade possui suas peculiaridades, como a capacidade de produzir determinado tipo deproduto, a produtividade, a disponibilidade e seus custos. Os dados disponíveis para o próximo trimestre seguem a seguir
...
# Em  função  do  nível  tecnológico  e  da  capacidade  da  mão  de  obra,  cada  fábrica  possui produtividades  diferentes  (peças/hora).  Além  disso,  em  função  de  leis trabalhistas,  cada fábrica possui uma disponibilidade (dias).
...
#As diferentes regiões possuem demandas mínimas que a empresa deve atender, em função de contratos já estabelecidos. Além disso, cada região determina qual o máximo de demanda poderia  ser  estabelecida,  caso  haja disponibilidade  e  seja  lucrativo  para  a  empresa.  A demanda é definida em milhares de peças (kpeças).
...
# PREÇO POR REGIAO
#Respeitando as diferenças econômicas de cada região, as equipes de pricingda empresa já determinaram os preços de cada produto em cada região ($/peça).
 param pr : 
        VEST    CAL     ACES :=
    NAM 21      72.2    88.5
    SAM 15      64.6    70.8
...
#A utilização ou não de uma fábrica no trimestre acarreta custos fixos, expressos em milhões de dólares (M$). Além disso, a produção de uma peça de cada produto possui um custo variável diferente em cada fábrica, expresso em dólares por peça ($/peça).
...
# CUSTOS LOGISTICOS
#O custo de transporte de uma peça entre uma fábrica e uma determinada região é expresso em dólares por peça ($/peça).
param cl :
...
# CAPACIDADE MAXIMA DE ESCOAMENTO
# Por questões políticas e limitações logísticas, existe um limite máximo de escoamento em cada rota, expresso em milhares de peças (kpeças)
param ce :
          NAM      SAM      CAM      EUR      ORM      AFR      OCE      NAS      SAS :=
